...
def run(args):
    all_paths, all_sentences = get_data_group_sources(args.dataset_path, language=args.language_id)
    ...
    if args.use_group_dro:
        test_data["group_idx"] = []
    group_str_to_idx = {source: idx for idx, source in enumerate(sources)}

    for source in sources: 
        for path, sentence in zip(all_paths[args.train_set][source], all_sentences[args.train_set][source]):
            train_data["audio"].append(path)
            train_data["sentence"].append(sentence)
            if args.use_group_dro:
                train_data["group_idx"].append(group_str_to_idx[source])
        for path, sentence in zip(all_paths["10mintest"][source], all_sentences["10mintest"][source]):
            test_data["audio"].append(path)
            test_data["sentence"].append(sentence)
            if args.use_group_dro:
                test_data["group_idx"].append(group_str_to_idx[source])
    ...
    if args.use_group_dro:
        group_idx_to_str = {idx: source for idx, source in enumerate(sources)}
        def group_str_fn(group_idx):
            return group_idx_to_str[group_idx]
        trainer = Seq2SeqDROTrainer(
            n_groups=len(sources),
            group_counts=torch.LongTensor([len(all_paths[args.train_set][source]) for source in sources]),
    ...
    for source in sources: 
        ...
        print(f"{source} Average WER: {average_wer}")
        average_wers[source] = average_wer

        all_wer_scores.extend(wer_scores)
    print(f"Overall Average WER: {np.mean(all_wer_scores)}")

    model_name = MODEL_ID.split("/")[-1] + "_" + LANG + "_" + args.train_set + "_" + "_".join(sources) + "_WER_" + str(np.mean(all_wer_scores)) + '_' + str(args.steps) + '_group_dro_' + str(args.use_group_dro) + ".pt"
    ...
    with open(os.path.join(args.output_dir, model_name.replace(".pt", ".txt")), "w") as f:
        for source, avg_wer in average_wers.items():
            f.write(f"{source} {avg_wer}\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--processor_language', default="english", type=str)
    parser.add_argument('--language_id', default="eng", type=str)
    parser.add_argument('--model_id', default="openai/whisper-tiny", type=str)
    parser.add_argument('--sources', nargs='+', help='languages to include in train/test set', default=["commonvoice", "voxforge"])
